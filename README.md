...
* Markdown Preview Mermaid Support (para visualizar diagramas en el editor)
...
### 5. Python 3.11+ (runtime para Spec Kit CLI)

Spec Kit CLI es una herramienta de línea de comandos escrita en Python. No se usa dentro de VS Code, solo en la terminal.
...
UV (instalador recomendado): https://github.com/astral-sh/uv
...
- Estructura de Carpetas: Todo el código fuente debe ubicarse dentro de src/ en la raíz del proyecto. Estructura: src/BankingApi/
...
> **Workaround conocido:** Las instrucciones de idioma y estructura de carpetas se repiten en los prompts de las partes 3, 4 y 5. Esto es intencional. En chats largos, el agente puede perder contexto de la constitución. La repetición actúa como refuerzo. Si notas que Copilot respeta el idioma y la estructura sin necesidad de repetirlo, puedes omitir esas líneas en los prompts siguientes.
...
```
REFUERZO (omitir si Copilot ya respeta la constitución):
- Documentación en español, código en inglés, estructura en src/BankingApi/
...
## Lecciones opcionales incluidas en este repositorio

Este taller incluye dos guías opcionales para quienes quieran explorar capacidades adicionales de Spec Kit fuera del flujo principal de 3 horas:

* **[OPTIONAL-SPEC-CRITIQUE.md](OPTIONAL-SPEC-CRITIQUE.md)** - Usar `/speckit.analyze` y `/speckit.checklist` para detectar errores de congruencia, ambiguedades y huecos de cobertura en los documentos de especificación generados.
* **[OPTIONAL-AZURE-DEVOPS-BOARDS.md](OPTIONAL-AZURE-DEVOPS-BOARDS.md)** - Integrar Azure DevOps Boards para leer historias de usuario existentes y sincronizar tareas generadas por Spec Kit hacia el board.
...
* **El agente pierde contexto de idioma o estructura:** Si después de varios mensajes Copilot empieza a generar en inglés o ignora la estructura de carpetas, agrega las líneas de REFUERZO al inicio de tu siguiente prompt. Esto es un comportamiento conocido en chats largos.
